...
//------------------------------------------
namespace Nebulite::Utility::Generate {
/**
 * @brief Generates a class C from a given sequence.
 * @tparam C The class to generate.
 * @tparam T The storage type inside the class.
 * @tparam Is The index sequence.
 * @tparam F The generator function type.
 * @param generator The generator function.
 * @return The generated class C.
 */
template<class C, typename T, std::size_t... Is, typename F>
C constexpr fromSequence(std::index_sequence<Is...> /*is*/, F&& generator) {
    return {
        {
...
constexpr std::array<T, N> array(F&& generator) {
    return fromSequence<std::array<T, N>,T>(
...
constexpr std::string string(F&& generator) {
    return fromSequence<std::string, char>(
        std::make_index_sequence<N>{},
        std::forward<F>(generator)
    );
}
...
/**
 * @brief Generation policy for Inclusion/exclusion
 */
 ...
 *         [2, 4, 8, ..., N] if ZeroPower = Policy::excluded
 *         [1, 2, 4, ..., N] if ZeroPower = Policy::included
 *         Where N is the highest power of 2 smaller or equal to ceiling.
 *
 */
template<Policy ZeroPower = Policy::excluded>
auto constexpr powersOfTwo(std::size_t const ceiling) {
...
auto constexpr iota(std::size_t const start, std::size_t const exclusiveMax) {
 ...
 */
template<typename ReturnType = std::size_t>
auto constexpr indices(std::size_t const exclusiveMax) {
    if (exclusiveMax == 0) {
