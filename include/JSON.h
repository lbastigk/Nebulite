...
namespace Nebulite{
     ...
     * 
     * @note Make sure to update this template and Nebulite::JSON::CacheEntry if the list of supported types changes.
     */
    template <typename T>
    ...
    public:
         ...
         * 
         * In order to further speed up expression evaluation, Nebulite::JSON offers direct access to cached double values.
    ...
    private:
        ...
        //--------------------------------------------------------------------
        // Important Helper functions for direct document Access
...
T Nebulite::JSON::get(const char* key, const T defaultValue) {
    ...
    if constexpr (is_simple_value_v<T> || std::is_same_v<T, const char*>) {
        ...
        T tmp = Nebulite::JSON::DirectAccess::get<T>(key, defaultValue, doc);
        set(key,tmp);
        return tmp;
    }
    // Fallback to doc
    return Nebulite::JSON::DirectAccess::get<T>(key, defaultValue, doc);
}
...
void Nebulite::JSON::set(const char* key, const T& value) {
    ...
    if constexpr (is_simple_value_v<T>) {
        ...
        cache.erase(key);
        Nebulite::JSON::DirectAccess::set<T>(key, value, doc, doc.GetAllocator());
    }
}
...
T Nebulite::JSON::DirectAccess::get(const char* key, const T defaultValue, rapidjson::Value& val) {
    rapidjson::Value* keyVal = Nebulite::JSON::DirectAccess::traverse_path(key,val);
    if(keyVal == nullptr){
        ...
        T tmp;
        Nebulite::JSON::DirectAccess::ConvertFromJSONValue<T>(*keyVal, tmp, defaultValue);
        return tmp;
    }
}

template <typename T>
void Nebulite::JSON::DirectAccess::set(const char* key, const T& value, rapidjson::Value& val, rapidjson::Document::AllocatorType& allocator) {
    // Ensure key path exists
    rapidjson::Value* keyVal = Nebulite::JSON::DirectAccess::ensure_path(key, val, allocator);
    if (keyVal != nullptr) {
        Nebulite::JSON::DirectAccess::ConvertToJSONValue<T>(value, *keyVal, allocator);
    } else {
