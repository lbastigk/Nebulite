...
public:
    ...
    Nebulite::ERROR_TYPE connect(int argc, char* argv[]);          // Establish connection: connect [protocol] [address] [port]
    Nebulite::ERROR_TYPE disconnect(int argc, char* argv[]);       // Close connection: disconnect
    Nebulite::ERROR_TYPE reconnect(int argc, char* argv[]);        // Reconnect using last settings: reconnect

    //----------------------------------------
    // Status Monitoring

    // Connection status and health
    Nebulite::ERROR_TYPE status(int argc, char* argv[]);           // Show connection status: status
    Nebulite::ERROR_TYPE ping(int argc, char* argv[]);             // Test connection: ping
    Nebulite::ERROR_TYPE isConnected(int argc, char* argv[]);      // Check if connected: is-connected

    //----------------------------------------
    // Command Parsing and Execution

    // Command handling
    Nebulite::ERROR_TYPE parseCommand(int argc, char* argv[]);     // Parse incoming command: parse-command <command_string>
    Nebulite::ERROR_TYPE executeRemote(int argc, char* argv[]);    // Execute command from remote: execute-remote <command>
    Nebulite::ERROR_TYPE setCommandHandler(int argc, char* argv[]); // Set command callback: set-command-handler <callback_command>

    //----------------------------------------
    // Image Transfer

    // Image sending functionality
    Nebulite::ERROR_TYPE sendImage(int argc, char* argv[]);        // Send image of current renderer to connected client.
