...
int Nebulite::mainTreeFunctions::json_test(int argc, char** argv){
    ...
    if(test_active == 0 | test_active == testCount){
        ...
        std::cout << "Set count is: " << count << std::endl << std::endl;
        std::cout << "Setting a nested double value, old: ";
        start = Time::gettime();
        ...
        std::cout << "Setting a nested double value, new: ";
        start = Time::gettime();
        for(volatile uint64_t i = 0; i < count; i++){
            (void) json.set<double>("global.time.t",1.2345);
        }
        std::cout << "\t Took " << Time::getruntime(start) << std::endl;
        std::cout << "Getting a nested double value, old: ";
        start = Time::gettime();
        for(volatile uint64_t i = 0; i < count; i++){
            (void) JSONHandler::Get::Any<double>(doc,"global.time.t",1.2345);
        }
        std::cout << "\t Took " << Time::getruntime(start) << std::endl;
        std::cout << "Getting a nested double value, new: ";
        start = Time::gettime();
        for(volatile uint64_t i = 0; i < count; i++){
    ...
    if(test_active == 0 | test_active == testCount){
        ...
        Nebulite::JSON json2;
        json2.set<double>("global.value1",123456);
        json2.set<double>("global.arr[2]",1.2345);
        std::cout << "global.value1 was set to 123456 (double)" << std::endl;
        std::cout << "global.arr[1] was set to 1.2345 (double)" << std::endl;
        std::cout << "Before flush by serialization:" << std::endl;
        ...
        std::cout << "After flush by serialization:" << std::endl;
        std::cout << "global.value1 = " << json2.get<double>("global.value1",0.0) << std::endl;
        std::cout << "global.arr[0] = " << json2.get<double>("global.arr[0]",0.0) << std::endl;
        std::cout << "global.arr[1] = " << json2.get<double>("global.arr[1]",0.0) << std::endl;
        std::cout << "global.arr[2] = " << json2.get<double>("global.arr[2]",0.0) << std::endl;
    ...
    if(test_active == 0 | test_active == testCount){
        ...
        //------------------------------------------------------------------
        std::cout << std::endl;
        std::cout << "Test "<< testCount << ": Testing type cache access" << std::endl;
        Nebulite::JSON type_test;
        type_test.set<int>("val",123);
        std::cout << "val was set to 123 as int " << std::endl;
        std::cout << "accessing as int, double and string:" << std::endl;
        std::cout << type_test.get<int>("val") << " | " << type_test.get<double>("val") << " | " << type_test.get<std::string>("val") << std::endl;
        type_test.set<double>("val",1.23);
        std::cout << "val was set to 1.23 as double" << std::endl;
        std::cout << "accessing as int, double and string:" << std::endl;
        std::cout << type_test.get<int>("val") << " | " << type_test.get<double>("val") << " | " << type_test.get<std::string>("val") << std::endl;
    ...
    if(test_active == 0 | test_active == testCount){
        ...
        Nebulite::JSON string_test;

        std::cout << std::endl;
        std::cout << "Setting num 12345.67 as string." << std::endl;
        string_test.set<std::string>("value","12345.67");
        std::cout << "Access as string: " << string_test.get<std::string>("value")  << std::endl;
        std::cout << "Access as int:    " << string_test.get<int>("value")          << std::endl;
        std::cout << "Access as double: " << string_test.get<double>("value")       << std::endl;
        std::cout << "Access as string: " << string_test.get<std::string>("value")  << std::endl;
        std::cout << "Access as int:    " << string_test.get<int>("value")          << std::endl;
        std::cout << "Access as double: " << string_test.get<double>("value")       << std::endl;
        ...
        std::cout << std::endl;
        std::cout << "Setting num 12345.67 as double." << std::endl;
        string_test.set<double>("value",12345.67);
        std::cout << "Access as string: " << string_test.get<std::string>("value")  << std::endl;
        std::cout << "Access as int:    " << string_test.get<int>("value")          << std::endl;
        std::cout << "Access as double: " << string_test.get<double>("value")       << std::endl;
        std::cout << "Access as string: " << string_test.get<std::string>("value")  << std::endl;
        ...
        std::cout << std::endl;
        std::cout << "3.) Setting a non-numeric string:" << std::endl;
        string_test.set<std::string>("value","ABC");
        std::cout << "Access as string: " << string_test.get<std::string>("value")  << std::endl;
        std::cout << "Access as int:    " << string_test.get<int>("value")          << std::endl;
        std::cout << "Access as double: " << string_test.get<double>("value")       << std::endl;
        std::cout << "Access as string: " << string_test.get<std::string>("value")  << std::endl;
