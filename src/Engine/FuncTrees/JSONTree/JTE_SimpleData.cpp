...
Nebulite::ERROR_TYPE Nebulite::JSONTreeExpansion::SimpleData::move(int argc, char* argv[]){
    ...
    if(domain->memberCheck(sourceKey) == Nebulite::JSON::KeyType::null) {
        std::cerr << "Error: Source key '" << sourceKey << "' does not exist." << std::endl;
        return Nebulite::ERROR_TYPE::UNKNOWN_ARG;
    }
    if(domain->memberCheck(sourceKey) == Nebulite::JSON::KeyType::document) {
        Nebulite::JSON subdoc = domain->get_subdoc(sourceKey.c_str());
        domain->remove_key(targetKey.c_str());
        domain->set_subdoc(targetKey.c_str(), subdoc);
        domain->remove_key(sourceKey.c_str());
    }
    else if (domain->memberCheck(sourceKey) == Nebulite::JSON::KeyType::array) {
...
Nebulite::ERROR_TYPE Nebulite::JSONTreeExpansion::SimpleData::ensureArray(int argc, char* argv[]) {
    ...
    Nebulite::JSON::KeyType keyType = domain->memberCheck(key);
...
Nebulite::ERROR_TYPE Nebulite::JSONTreeExpansion::SimpleData::push_back(int argc, char* argv[]){
    ...
    if (domain->memberCheck(key) != Nebulite::JSON::KeyType::array) {
...
Nebulite::ERROR_TYPE Nebulite::JSONTreeExpansion::SimpleData::push_front(int argc, char* argv[]){
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        Nebulite::JSON::KeyType itemType = domain->memberCheck(itemKey);
