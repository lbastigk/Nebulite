...
//-------------------------------
// Update
void Nebulite::GlobalSpaceTreeExpansion::GUI::update() {
    // Add FuncTree-specific updates here!
    // General rule:
    // This is used to update all variables/states that are INTERNAL ONLY

    // We can also update internal states from the local level:
    // int displaySizeX = self->global.get<int>(keyName.renderer.dispResX.c_str(), 0);
}


Nebulite::ERROR_TYPE Nebulite::GlobalSpaceTreeExpansion::GUI::example(int argc, char* argv[]) {
    ...
    self->getRenderer()->attachTextureAboveLayer(
        Nebulite::Environment::Layers::menue, 
        "circle_texture", 
