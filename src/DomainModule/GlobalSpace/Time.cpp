...
Constants::Error Time::update() {
    ...
    // Set in doc
    domain->getDoc()->set<double>(Key::runtime_dt, static_cast<double>(dt_ms) / 1000.0);
    domain->getDoc()->set<double>(Key::runtime_t, static_cast<double>(t_ms) / 1000.0);
    domain->getDoc()->set<Uint64>(Key::runtime_dt_ms, dt_ms);
    ...
    if (!haltThisFrame && timeLocks.empty() && !domain->getRenderer()->isSkippingUpdate()) {
        ...
        // Set in doc
        domain->getDoc()->set<double>(Key::time_dt, static_cast<double>(sim_dt_ms) / 1000.0);
        domain->getDoc()->set<double>(Key::time_t, static_cast<double>(sim_t_ms) / 1000.0);
        domain->getDoc()->set<Uint64>(Key::time_dt_ms, sim_dt_ms);
        domain->getDoc()->set<Uint64>(Key::time_t_ms, sim_t_ms);
