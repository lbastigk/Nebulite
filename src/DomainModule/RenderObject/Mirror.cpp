...
Constants::Error Mirror::update() {
    if (mirrorEnabled || mirrorOnceEnabled) {
        ...
        auto const globalScope = globalDoc().shareScope(*this).getRootScope();
        globalDoc().shareScope(*this).setSubDoc(globalScope + mirrorKey, moduleScope);
...
Constants::Error Mirror::mirror_delete() const {
    auto const globalScope = globalDoc().shareScope(*this).getRootScope();
    globalDoc().shareScope(*this).removeKey(globalScope + mirrorKey);
    return Constants::ErrorTable::NONE();
}

Constants::Error Mirror::mirror_fetch() const {
    auto const globalScope = globalDoc().shareScope(*this).getRootScope();
    if (globalDoc().shareScope(*this).memberType(globalScope + mirrorKey) != Data::KeyType::object) {
        return Constants::ErrorTable::addError("Mirror fetch failed: Key '" + mirrorKey + "' not of type document", Constants::Error::NON_CRITICAL);
    }
    domain.deserialize(globalDoc().shareScope(*this).serialize(globalScope + mirrorKey));
    return Constants::ErrorTable::NONE();
}
