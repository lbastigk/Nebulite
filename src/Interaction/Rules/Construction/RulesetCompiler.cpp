...
void RulesetCompiler::getFunctionCalls(Core::JsonScope& entryDoc, JsonRuleset& Ruleset, Execution::DomainBase const& self) {
    ...
    if (entryDoc.memberType(Constants::KeyNames::Ruleset::parseOnGlobal) == Data::KeyType::array) {
        ...
        for (size_t j = 0; j < funcSize; ++j) {
            ...
            Logic::ExpressionPool invokeExpr(funcCall, self.getDoc());
...
void RulesetCompiler::setMetaData(
    ...
    // Set IDs
    auto const id = self.getDoc().get<uint32_t>(Constants::KeyNames::RenderObject::id, 0);
...
void RulesetCompiler::parse(std::vector<std::shared_ptr<Ruleset>>& rulesetsGlobal, std::vector<std::shared_ptr<Ruleset>>& rulesetsLocal, Interaction::Execution::DomainBase& self) {
    ...
    for (size_t idx = 0; idx < size; ++idx) {
        ...
        auto Ruleset = getRuleset(self.getDoc(), key.view(), self);
...
RulesetCompiler::AnyRuleset RulesetCompiler::getRuleset(Core::JsonScope const& doc, Data::ScopedKeyView const& key, Execution::DomainBase& self) {
    ...
    Ruleset->logicalArg = std::make_unique<Logic::ExpressionPool>(logicalArgStr, self.getDoc());
    ...
    for (auto& assignment : Ruleset->assignments) {
        assignment.expression = std::make_unique<Logic::ExpressionPool>(assignment.value, self.getDoc());
    }
