...
void Debug::who(ContextBase const& context) {
    uint32_t slf = context.self.getDoc()->get<uint32_t>(Nebulite::Constants::KeyNames::RenderObject::id, 0);
    uint32_t otr = context.other.getDoc()->get<uint32_t>(Nebulite::Constants::KeyNames::RenderObject::id, 0);
